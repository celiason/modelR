...
(define-param n4 1.56)           ; substrate RI
(define e1 (* n1 n1)) ; permittivity of keratin
(define e2 (* n2 n2)) ; permittivity of melanin
(define e3 (* n3 n3)) ; subtrait refractive index
(define e4 (* n4 n4)) ; subtrait refractive index
(define-param k1 0.05)         ; layer 1 extinction coef
(define-param k2 0.05)         ; layer 2 extinction coef
(define-param k3 0.05)        ; substrate extinction coef
(define-param k4 0.05)        ; substrate extinction coef
...
(define mat4 (make medium 
                (epsilon e4) 
                (D-conductivity (/ (* 2 pi fcen (* 2 (sqrt e4) k4)) e4))))
        ...
        (append
          ...
          (geometric-object-duplicates (vector3 a 0) 0 (- nx 1)
                (make block (center (+ dpml pad cor d1 (/ d2 2) (* -0.5 sx)) 0)
                            ...
                            (material mat2)))
          (geometric-object-duplicates (vector3 a 0) 0 (- nx 1)
                (make block (center (+ dpml pad cor d1 d2 (/ d3 2) (* -0.5 sx)) 0)
                            (size d3 sy infinity)
                            (material mat3)))
          )))
  ...
  (set! geometry	
    (if no-struc?
      ...
      (append
        (list
          (make block (center 0 0) (size sx sy infinity) (material air))
          (make block (center (+ dpml pad (/ (+ cor sx1 sub) 2) (* -0.5 sx)) 0)
                            (size (+ cor sx1 sub) sy infinity)
                            (material mat1)))
        (geometric-object-duplicates (vector3 a 0) 0 (- nx 1)
              (make block (center (+ dpml pad cor (/ d1 2) (* -0.5 sx)) 0)
                          (size d1 sy infinity)
