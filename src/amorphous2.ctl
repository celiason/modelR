...
(define-param sub 2)  ; air substrate
(define-param dpml 1) ; width of absorbing boundary layers
(define-param d0 .08) ; unscaled particle diameter (from molec. dynam. simulations)
(define-param cor 0)  ; cortex
...
(define-param bleed? false)  ; whether to have cortex "bleed" in-between melanosomes or not
      ...
      (set! geometry
        (append geometry
           (list
             (make cylinder 
                (center 
                  (+ (* (vector-ref xx x) (cos rot) mag) (* (vector-ref yy x) mag (sin rot))) ; x
                  ; (- (* 0.5 sy) dpml pad cor (/ (* sy0 mag) 2) (+ (* (* (vector-ref xx x) -1) (sin rot) mag) (* (vector-ref yy x) mag (cos rot))))) ; y
                  (- (+ (* (* (vector-ref xx x) -1) (sin rot) mag) (* (vector-ref yy x) mag (cos rot))) (/ (+ (- pad sub) r cor) 2))) ; y
                (radius r)
                  (height infinity) (material rod))
              (make cylinder 
                (center 
                  (+ (* (vector-ref xx x) (cos rot) mag) (* (vector-ref yy x) mag (sin rot))) ; x
                  (- (* 0.5 sy) dpml pad cor (+ (* (* (vector-ref xx x) -1) (sin rot) mag) (* (vector-ref yy x) mag (cos rot))))) ; y
                    (radius r2)
...
(if no-struc?
  ...
  (set! geometry
    (list 
     (make block
        (center 0 (- (* 0.5 sy) (/ (+ dpml pad) 2)))
           (size sx (+ pad cor r)) (material outside))
      (make block
         (center 0 (+ (- (* 0.5 sy) dpml pad (/ cor 2) (/ bleed 2)) r))
           (size sx (+ cor bleed) infinity) (material cortex)))))
